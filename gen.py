...
for rows in points[:-1]:
    ...
    for columns in rows[:-1]:
        ...
        r = str(min(255, int(R_START + r_dif * d + R_RANDOMNESS * (2 * random.random() - 1 ) ** 2)))
        g = str(min(255, int(G_START + g_dif * d + G_RANDOMNESS * (2 * random.random() - 1 ) ** 2)))
        b = str(min(255, int(B_START + b_dif * d + B_RANDOMNESS * (2 * random.random() - 1 ) ** 2)))
